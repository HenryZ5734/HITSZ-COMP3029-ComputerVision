...
def resize_image(input_image, output_rows, output_cols):
    ...
    for i in range(output_rows):
        for j in range(output_cols):
            output_image[i, j, :] = input_image[int(i/output_rows*input_rows), int(j/output_cols*input_cols), :]
    ### END YOUR CODE
...
def rotate2d(point, theta):
    ...
    ## YOUR CODE HERE
    rotateMat = np.array([np.cos(theta), -np.sin(theta), np.sin(theta), np.cos(theta)]).reshape(2, 2)
    ans = rotateMat @ point.reshape(2, 1)
...
def rotate_image(input_image, theta):
    ...
    rotate_mat = np.array([np.cos(theta), -np.sin(theta), np.sin(theta), np.cos(theta)]).reshape(2, 2)
    for i in range(input_rows):
        for j in range(input_cols):
            coordinate = np.array([i, j]) - origin
            input_cor = rotate_mat @ coordinate.reshape(2, 1)
            input_cor = input_cor.reshape(-1) + origin
            if 0 <= input_cor[0] and input_cor[0] < input_rows and 0 <= input_cor[1] and input_cor[1] < input_cols:
                output_image[i, j, :] = input_image[int(input_cor[0]), int(input_cor[1]), :]
    ### END YOUR CODE
